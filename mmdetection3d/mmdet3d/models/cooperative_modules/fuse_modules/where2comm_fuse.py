"""Where2comm communication-guided fusion.

forward(x, psm_single, record_len, pairwise_t_matrix): returns ((B, C, H, W), ...).
"""
import random
...
class Communication(nn.Module):
    def __init__(self, threshold, gaussian_smooth=None):
        ...
        if gaussian_smooth is not None:
            ...
            self.gaussian_filter = nn.Conv2d(
                1, 1, kernel_size=kernel_size, stride=1,
                padding=(kernel_size - 1) // 2)
            self._init_gaussian_filter(kernel_size, c_sigma)
            self.gaussian_filter.requires_grad = False
        else:
            self.smooth = False

    def _init_gaussian_filter(self, k_size=5, sigma=1.0):
        center = k_size // 2
        x, y = np.mgrid[0 - center:k_size - center,
                         0 - center:k_size - center]
        gaussian_kernel = (1 / (2 * np.pi * sigma) *
    ...
    def forward(self, batch_confidence_maps, B):
        ...
        communication_rates = []
        for b in range(B):
            ori_communication_maps, _ = batch_confidence_maps[b].sigmoid() \
            ...
            if self.training:
                ...
                ones_fill = torch.ones(
                    L, K, dtype=communication_maps.dtype,
...
class Where2comm(nn.Module):
    def __init__(self, voxel_size, downsample_rate, fully, multi_scale,
                 communication, in_channels=256, layer_nums=None,
                 num_filters=None):
        ...
        if self.multi_scale:
            assert layer_nums is not None and num_filters is not None
        ...
        self.naive_communication = Communication(**communication)
    ...
    def forward(self, x, psm_single, record_len, pairwise_t_matrix,
                backbone=None):
        _, C, H, W = x.shape
