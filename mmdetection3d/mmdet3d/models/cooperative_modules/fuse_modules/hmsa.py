"""Heterogeneous multi-scale attention (used by V2X-ViT).
"""
import torch
...
class HGTCavAttention(nn.Module):
    def __init__(self, dim, heads, num_types=2, num_relations=4,
    ...
    def to_qkv(self, x, types):
        ...
        for b in range(x.shape[0]):
            ...
            for i in range(x.shape[-2]):
                q_list.append(
                    self.q_linears[types[b, i]](
                        x[b, :, :, i, :].unsqueeze(2)))
                k_list.append(
                    self.k_linears[types[b, i]](
                        x[b, :, :, i, :].unsqueeze(2)))
                v_list.append(
                    self.v_linears[types[b, i]](
                        x[b, :, :, i, :].unsqueeze(2)))
    ...
    def get_hetero_edge_weights(self, x, types):
        w_att_batch, w_msg_batch = [], []
        for b in range(x.shape[0]):
            w_att_list, w_msg_list = [], []
            for i in range(x.shape[-2]):
                w_att_i_list, w_msg_i_list = [], []
                for j in range(x.shape[-2]):
                    e_type = self.get_relation_type_index(
                        types[b, i], types[b, j])
                    w_att_i_list.append(
                        self.relation_att[e_type].unsqueeze(0))
                    w_msg_i_list.append(
                        self.relation_msg[e_type].unsqueeze(0))
                w_att_list.append(
                    torch.cat(w_att_i_list, dim=0).unsqueeze(0))
                w_msg_list.append(
                    torch.cat(w_msg_i_list, dim=0).unsqueeze(0))
            w_att_batch.append(
                torch.cat(w_att_list, dim=0).unsqueeze(0))
            w_msg_batch.append(
                torch.cat(w_msg_list, dim=0).unsqueeze(0))
    ...
    def forward(self, x, mask, prior_encoding):
        ...
        q, k, v = map(
            lambda t: rearrange(t, 'b h w l (m c) -> b m h w l c',
                                m=self.heads), qkv)
        ...
        v_msg = torch.einsum(
            'b m i j p c, b m h w j p -> b m h w i j c', w_msg, v)
        out = torch.einsum(
            'b m h w i j, b m h w i j c -> b m h w i c', att_map, v_msg)
