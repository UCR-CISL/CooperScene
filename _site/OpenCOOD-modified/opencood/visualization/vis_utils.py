...
def visualize_inference_sample_dataloader(batch_data,
                                          pred_box_tensor,
                                          gt_box_tensor,
                                          origin_lidar,
                                          o3d_pcd,
    ...
    gt_ids = batch_data['ego'].get('object_ids', None)
    gt_spheres = make_id_spheres(gt_box_tensor, ids=gt_ids)

    return o3d_pcd, pred_o3d_box, gt_o3d_box, gt_spheres
...
def visualize_sequence_dataloader(dataloader, order, color_mode='constant',
    ...
    coordinate_frame = o3d.geometry.TriangleMesh().create_coordinate_frame(size=50.0)
    ...
    while True:
        for i_batch, sample_batched in enumerate(dataloader):
            ...
            vis.poll_events()
            vis.update_renderer()
            time.sleep(0.017) # roughly 60 fps
...
def make_id_spheres(bbx_corner, ids=None, radius=1.5):
    """
    Place spheres at bbox centers as ID markers.
    """
    ...
    for i in range(len(bbx_corner)):
        ...
        if ids is not None:
            color = plt.get_cmap("tab20")(ids[i] % 23)[:3]
        else:
            color = (1.0, 1.0, 0.0)
