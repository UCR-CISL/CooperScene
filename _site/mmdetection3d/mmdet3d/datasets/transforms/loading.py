...
class LoadPointsFromPCD(LoadPointsFromFile):
    """Load Points From PCD file (Open3D format).

    This is used for datasets like OPV2V that store point clouds in .pcd format.

    Required Keys:

    - lidar_points (dict)

        - lidar_path (str): Path to .pcd file
    ...
    Args:
        coord_type (str): The type of coordinates of points cloud.
            Available options includes:
            - 'LIDAR': Points in LiDAR coordinates.
            - 'DEPTH': Points in depth coordinates.
            - 'CAMERA': Points in camera coordinates.
        load_dim (int): The dimension of the loaded points. Defaults to 4.
        use_dim (list[int] | int): Which dimensions of the points to use.
            Defaults to [0, 1, 2, 3] for x, y, z, intensity.
        backend_args (dict, optional): Arguments to instantiate the
            corresponding backend. Defaults to None.
    """
    ...
    def _load_points(self, pts_filename: str) -> np.ndarray:
        """Private function to load point clouds data from .pcd file.
        ...
        try:
            ...
            raise ImportError(
                'Please install open3d to load .pcd files: pip install open3d')

        # Read PCD file
        pcd = o3d.io.read_point_cloud(pts_filename)
        points = np.asarray(pcd.points, dtype=np.float32)  # (N, 3)

        # Handle intensity/color
        if pcd.colors is not None and len(pcd.colors) > 0:
            # Use grayscale of colors as intensity
            colors = np.asarray(pcd.colors, dtype=np.float32)
            intensity = (colors[:, 0] * 0.299 +
                        colors[:, 1] * 0.587 +
                        colors[:, 2] * 0.114).reshape(-1, 1)
        else:
            # No intensity available, use ones
            intensity = np.ones((points.shape[0], 1), dtype=np.float32)

        # Combine [x, y, z, intensity]
        points = np.hstack([points, intensity])

        # Pad to load_dim if needed
        if points.shape[1] < self.load_dim:
            padding = np.zeros(
                (points.shape[0], self.load_dim - points.shape[1]),
                dtype=np.float32)
