...
def _torch_inverse_cast(input):
    ...
    is cast input data type to fp32, apply torch.inverse on CPU,
    and cast back to the input dtype and device.
    Args:
        input : torch.Tensor
    ...
    if dtype not in (torch.float32, torch.float64):
        dtype = torch.float32
    # Move to CPU and convert dtype for stable inverse
...
# --- appended for CoSDH (Where2comm/att_fuse) ---
