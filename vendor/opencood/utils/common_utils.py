...
def compute_3d_iou(det_polygon, gt_polygons, det_box, gt_boxes):
    """3D IoU = BEV polygon intersection * z overlap / 3D union.

    Assumes upright boxes (rotation only around z), so the BEV polygon
    captures the full xy extent and z is read directly off the corners.

    Parameters
    ----------
    det_polygon : shapely.geometry.Polygon
        BEV polygon of one detection.
    gt_polygons : list[shapely.geometry.Polygon]
        BEV polygons of all matched GTs.
    det_box : np.ndarray
        (8, 3) corners of the detection box.
    gt_boxes : np.ndarray
        (M, 8, 3) corners of all GT boxes.

    Returns
    -------
    np.ndarray of length M, 3D IoU per GT.
    """
