...
class BaseDataset(Dataset):
    ...
    def __init__(self, params, visualize, train=True):
        ...
        # Handle the cooperative scene dataset
        if 'useENU' in params:
            self.useENU = params['useENU']
        # Handle the pcd file of (x, y, z, intensity)
        ...
        for (i, scenario_folder) in enumerate(scenario_folders):
            ...
            # Roadside unit data's id is treated specially: in OPV2V it is a
            # negative integer; in CooperScene we adopt agent_id == 0 for the
            # infrastructure. In either case, push it to the end so it is
            # never selected as the ego.
            if int(cav_list[0]) <= 0:
    ...
    def calc_dist_to_ego(self, scenario_database, timestamp_key):
        ...
        for cav_id, cav_content in scenario_database.items():
            if cav_content['ego']:
                ...
                if isinstance(ego_lidar_pose, np.ndarray) or (isinstance(ego_lidar_pose, list) and isinstance(ego_lidar_pose[0], list)):
                    ego_lidar_pose = self.lidar_matrix_to_pose(ego_lidar_pose)
        ...
        for cav_id, cav_content in scenario_database.items():
            cur_lidar_pose = \
                load_yaml(cav_content[timestamp_key]['yaml'])['lidar_pose']
            if isinstance(cur_lidar_pose, np.ndarray) or (isinstance(cur_lidar_pose, list) and isinstance(cur_lidar_pose[0], list)):
                cur_lidar_pose = self.lidar_matrix_to_pose(cur_lidar_pose)
    ...
    def reform_param(self, cav_content, ego_content, timestamp_cur,
        ...
        if isinstance(cur_params['lidar_pose'], np.ndarray) or (isinstance(cur_params['lidar_pose'], list) and isinstance(cur_params['lidar_pose'][0], list)):
            cur_params['lidar_pose'] = self.lidar_matrix_to_pose(cur_params['lidar_pose'])
            
        if isinstance(delay_params['lidar_pose'], np.ndarray) or (isinstance(delay_params['lidar_pose'], list) and isinstance(delay_params['lidar_pose'][0], list)):
            delay_params['lidar_pose'] = self.lidar_matrix_to_pose(delay_params['lidar_pose'])
            
        if isinstance(cur_ego_params['lidar_pose'], np.ndarray) or (isinstance(cur_ego_params['lidar_pose'], list) and isinstance(cur_ego_params['lidar_pose'][0], list)):
            cur_ego_params['lidar_pose'] = self.lidar_matrix_to_pose(cur_ego_params['lidar_pose'])
            
        if isinstance(delay_ego_params['lidar_pose'], np.ndarray) or (isinstance(delay_ego_params['lidar_pose'], list) and isinstance(delay_ego_params['lidar_pose'][0], list)):
            delay_ego_params['lidar_pose'] = self.lidar_matrix_to_pose(delay_ego_params['lidar_pose'])

        # for key in cur_params['vehicles']:
        #     car_i = cur_params['vehicles'][key]
        #     car_i['angle'] = list(np.degrees(car_i['angle']))
        #     car_i['location'] = list(np.array(car_i['location'])+cur_params['lidar_pose'][:3])

        # for key in delay_params['vehicles']:
        #     car_i = delay_params['vehicles'][key]
        #     car_i['angle'] = list(np.degrees(car_i['angle']))
        #     car_i['location'] = list(np.array(car_i['location'])+delay_params['lidar_pose'][:3])
        
        # for key in cur_ego_params['vehicles']:
        #     car_i = cur_ego_params['vehicles'][key]
        #     car_i['angle'] = list(np.degrees(car_i['angle']))
        #     car_i['location'] = list(np.array(car_i['location'])+cur_ego_params['lidar_pose'][:3])
        
        # for key in delay_ego_params['vehicles']:
        #     car_i = delay_ego_params['vehicles'][key]
        #     car_i['angle'] = list(np.degrees(car_i['angle']))
        #     car_i['location'] = list(np.array(car_i['location'])+delay_ego_params['lidar_pose'][:3])
    ...
    def lidar_matrix_to_pose(self, matrix):
        """
