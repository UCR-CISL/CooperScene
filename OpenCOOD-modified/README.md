# CooperScene Modified
## New parameters in `hypes_yaml`:
- `intensity`: Boolean. If the pcd is in (x,y,z,intensity) format, set it to True; If it is in (x, y, z, color) format, set it to False. Other formats are not supported.
- `filter_lidar_points`: Boolean. If set to True, the points that farther than certain distance of the ego vehicle will be filtered out. 
- `useENU:` Boolean. If set to True, the read-in lidar pose from the pcd file will not do any transformation. 
