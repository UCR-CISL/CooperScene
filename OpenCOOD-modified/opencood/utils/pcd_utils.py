...
def pcd_to_np(pcd_file,isIntensity=False):
    """
    ...
    # Use the Open3D Tensor API to support (x,y,z,intensity) format
    pcd = o3d.t.io.read_point_cloud(pcd_file)

    xyz = np.asarray(pcd.point.positions.numpy())
    
    if isIntensity:
        intensity = np.asarray(pcd.point.intensity.numpy())
        # import pickle
        # with open('/home/cisl/Data/cooperscene_intensity.pkl','wb') as f:
        ...
        # import pickle
        # with open('/home/cisl/Data/opv2v_intensity_normalize.pkl','wb') as f:
        #     pickle.dump(np.expand_dims(np.asarray(pcd.point.colors.numpy()/255[:, 0]),-1),f)
        # exit()
        intensity = np.expand_dims(np.asarray(pcd.point.colors.numpy()/255)[:, 0], -1)
