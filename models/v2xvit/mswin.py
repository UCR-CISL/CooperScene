"""Multi-scale window attention (pyramid-window, used by V2X-ViT).
"""
import torch
...
from ..cooperative.sub_modules.split_attn import SplitAttn
...
class BaseWindowAttention(nn.Module):
    def __init__(self, dim, heads, dim_head, drop_out, window_size,
        ...
        self.to_out = nn.Sequential(
            nn.Linear(inner_dim, dim), nn.Dropout(drop_out))
    ...
    def forward(self, x):
        ...
        q, k, v = map(
            lambda t: rearrange(
                t,
                'b l (new_h w_h) (new_w w_w) (m c) '
                '-> b l m (new_h new_w) (w_h w_w) c',
                m=m, w_h=self.window_size, w_w=self.window_size), qkv)
        dots = torch.einsum(
            'b l m h i c, b l m h j c -> b l m h i j', q, k) * self.scale
        if self.relative_pos_embedding:
        ...
        out = torch.einsum(
            'b l m h i j, b l m h j c -> b l m h i c', attn, v)
        out = rearrange(
            out,
            'b l m (new_h new_w) (w_h w_w) c '
            '-> b l (new_h w_h) (new_w w_w) (m c)',
...
class PyramidWindowAttention(nn.Module):
    def __init__(self, dim, heads, dim_heads, drop_out, window_size,
        ...
        for (head, dim_head, ws) in zip(heads, dim_heads, window_size):
            self.pwmsa.append(BaseWindowAttention(
