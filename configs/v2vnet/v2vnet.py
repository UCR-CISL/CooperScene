...
opencood_args = dict(
    ...
    v2vfusion=dict(
        agg_operator='avg',
        conv_gru=dict(H=50, W=176, kernel_size=[[3, 3]], num_layers=1),
        downsample_rate=4,
        gru_flag=True,
        in_channels=256,
        num_iteration=2,
...
opencood_postprocess_args = dict(
    max_num=100, nms_thresh=0.15,
    target_args=dict(
        pos_threshold=0.6, neg_threshold=0.45, score_threshold=0.20),
)

opencood_loss_args = dict(cls_weight=1.0, reg=2.0)
...
train_dataloader = dict(
    batch_size=4, collate_fn=dict(type='cooperative_collate'),
    ...
    dataset=dict(
        ...
        pcd_limit_range=point_cloud_range, max_cav=5, com_range=70))
...
val_dataloader = dict(
    ...
    dataset=dict(
        ...
        test_mode=True, pcd_limit_range=gt_range, max_cav=5, com_range=70))
...
load_from = 'work_dirs/opencood_converted/v2vnet.pth'
